...
const Sidebar: React.FC<SidebarProps> = ({ isOpen }) => {
  return (
    ...
    >
      ...
      {/* Folders Section */}
      <div className={`p-4 pt-16 mt-8`}>
        <h2 className="text-lg font-bold mb-6">Folders</h2>
        {['Personal', 'Work', 'Family'].map((folder) => (
          <div key={folder} className="mb-3 p-2 bg-gray-100 rounded cursor-pointer hover:bg-gray-200">{folder}</div>
        ))}
      </div>
    </div>
  );
...
const SuggestedJourneys: React.FC = () => {
  ...
  return (
    <div className="mt-8 flex-shrink-0 overflow-x-auto scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-100 pb-2">
      <h3 className="text-sm font-semibold mb-4 text-center">Suggested Journeys</h3>
